...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 'w':
        ...
        break;
    case 'A':
        if (ambientRef < 1.f) { ambientRef += 0.1f; }
        break;
    case 'a':
        if (ambientRef > 0.f) { ambientRef -= 0.1f; }
        break;
    case 'D':
        if (diffuseRef < 1.f) { diffuseRef += 0.1f; }
        break;
    case 'd':
        if (diffuseRef > 0.f) { diffuseRef -= 0.1f; }
        break;
    case 'S':
        if (specularRef < 1.f) { specularRef += 0.1f; }
        break;
    case 's':
        if (specularRef > 0.f) { specularRef -= 0.1f; }
        break;
    case '+':
        if (shininess < 100.f) { shininess += 1.f; }
        break;
    case '-':
        if (shininess > 1.f) { shininess -= 1.f; }
        break;
    case '?':
    default:
        printUsage ();
