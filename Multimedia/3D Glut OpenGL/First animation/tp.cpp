...
void updateAnimation (){
    ...
    /*for(unsigned int i = 0; i<V.size() ; i++){
        V[i].position = (1-interpolant1) * V0[i].position + interpolant1 * V1[i].position;
    }*/
    ...
    Vec3 translation(offset,0,0); // A mettre à jour
    ...
    Mat3 model2 = Mat3(1,0,0,0,-1,0,0,0,-1);
    //Mettre a jour Rz pour avoir une rotation atour de l'axe z de angle
    model2 = model2 * Ry;

     for(unsigned int i = 0; i<V.size() ; i++){
        V[i].position = w0 * V0[i].position + w1 * V1[i].position + w2 * V2[i].position;
        V[i].position = Rx * V[i].position;
        V[i].position += translation;
        V[i].position = model2 * V[i].position;
    }
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 'R':
        ...
        interpolant2 = std::min( interpolant2 + 0.1f, 1.0f);
        updateAnimation();
        break;
    case 'r':
        angle -= 0.1f;
