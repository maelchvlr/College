...
int nbr_facettes;
//To complete
void setUnitSphere( Mesh & o_mesh, float r=10, float h=5, int nbr_facettes=20 )
{
  float step_theta = (2 * M_PI) / (nbr_facettes - 1);

  float x, y, z;

    // centre des deux cercles
    // bien retenir que les deux centres sont à l'index 0 et 1 (créés en premier) de l'array des vertex
    o_mesh.vertices.push_back(Vec3(0, 0, 0));
    o_mesh.vertices.push_back(Vec3(0, 0, h));
    o_mesh.normals.push_back(Vec3(0, 0, h));
    o_mesh.normals.push_back(Vec3(0, 0, h));

    for (int i = 0; i < nbr_facettes; i++) {
      float t = i * step_theta;
      ...
      /* Visualisation d'un maillage d'un cylindre. C'est une TRANCHE du cylindre
      (les triangles en haut et en bas sont pliés à 90 degrés sur le cercle, 0 et 1 sont les centre des cercles)

                           0    Attention a bien tous les faire tourner dans le meme sens
            0            / |
          / |           /  |
          2_4          j__j+2
          |/|   ou     | / |
     ...
     for (int j = 2; j < (nbr_facettes * 2) - 1; j+=2) {
      //maillage des triangles haut et bas (pour le centre des cercles)
      o_mesh.triangles.push_back(Triangle(j, 0, j+2));
      o_mesh.triangles.push_back(Triangle(j+1, j+3, 1));
      
      //maillage des cotés du cylindre
      o_mesh.triangles.push_back(Triangle(j, j+3, j+1));
      o_mesh.triangles.push_back(Triangle(j, j+2, j+3));
...
void increaseSphereResolution(){
    ...
    setUnitSphere(unit_sphere, 10, 5, CYLINDER_RES);
}
void decreaseSphereResolution(){
    unit_sphere.vertices.clear();
    unit_sphere.triangles.clear();
    unit_sphere.normals.clear();
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '+': //Increase unit sphere reoslution
        increaseSphereResolution();
        break;

    case '-': //Decrease unit sphere resolution
        decreaseSphereResolution();
        break;
